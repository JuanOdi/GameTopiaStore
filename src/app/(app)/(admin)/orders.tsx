...
export default function AdminOrdersScreen() {
  ...
  async function confirmDeleteAll() {
    const confirmed = Platform.OS === 'web'
      ? window.confirm('Are you sure you want to delete all orders? This cannot be undone.')
    ...
    if (!confirmed) return;
    try { await deleteAllOrders(); } catch (e: any) { Alert.alert('Error', e.message); }
  }
  ...
  async function confirmDeleteOrder(orderId: string) {
    ...
    if (!confirmed) return;
    try { await deleteOrder(orderId); } catch (e: any) { Alert.alert('Error', e.message); }
  }
  ...
  const completedCount = dateFiltered.filter(o => o.status === 'completed' || o.status === 'confirmed').length;
  ...
  type ListItem =
    | { type: 'header'; label: string; summaries: CustomerSummaryRow[]; key: string }
  ...
  function buildGrouped(list: Order[]): ListItem[] {
    ...
    const dateKeys = [...new Set(list.map(o => new Date(o.createdAt + 4 * 60 * 60 * 1000).toISOString().slice(0, 10)))];
    ...
    for (const dateKey of dateKeys) {
      ...
      const yestKey  = new Date(Date.now() + 4 * 60 * 60 * 1000 - 86400000).toISOString().slice(0, 10);
      ...
      const summaries = Object.values(custMap).sort((a, b) => b.total - a.total);

      result.push({ type: 'header', label, summaries, key: `h-${dateKey}` });
      dayOrders.forEach(order => result.push({ type: 'order', order, key: order.id }));
    }
    return result;
  }
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.headerRow}>
        ...
        <Pressable
          style={({ pressed }) => [styles.clearBtn, pressed && { transform: [{ scale: 0.97 }] }]}
            ...
            return (
              <View style={styles.card}>
                ...
                <View style={styles.cardCenter}>
                  ...
                  {/* Email */}
                  <Text style={styles.emailText} numberOfLines={1}>{order.userEmail}</Text>
                  ...
                  {order.status === 'cancelled' && order.cancelReason ? (
                    <Text style={styles.cancelReasonText}>Rejected: {order.cancelReason}</Text>
                  ) : null}
                  {order.addOns && order.addOns.length > 0 && (
                    <Text style={styles.addOnsText}>
                      +{order.addOns
                        .map((a) => ((a.quantity ?? 1) > 1
...
const styles = StyleSheet.create({
  ...
  unpaidReason: { color: C.muted, fontFamily: F.medium, fontSize: 12, marginTop: 3 },
  addOnsText: { color: C.blue, fontFamily: F.medium, fontSize: 12, marginTop: 3 },
  noteText: { color: C.muted, fontFamily: F.medium, fontSize: 12, marginTop: 2, fontStyle: 'italic' },
  cancelReasonText: { color: C.coral, fontFamily: F.medium, fontSize: 12, marginTop: 3 },
